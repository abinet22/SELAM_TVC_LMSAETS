...
router.post('/findmyclasstoevaluatecourselevel',ensureAuthenticated,async function(req,res){
    ...
    res.render('evaluationclasssearchresult',{  tag:"level" , course:course, batchinfo:batchinfo,classlist:results,batchid:batchid,dpt:dpt,programtag:"level" ,showtype:showtype})
});
...
router.post('/findmyclasstoevaluatecourseindustry',ensureAuthenticated,async function(req,res){
  ...
  const [results, metadata] = await sequelize.query(
   ...
   console.log(results)
  res.render('evaluationclasssearchresult',{ tag:"industry" ,  course:course, batchinfo:batchinfo,classlist:results,batchid:batchidi,dpt:dpti,programtag:"industry",showtype:showtype})
});
router.post('/findmyclasstoevaluatecoursengo',ensureAuthenticated,async function(req,res){
  ...
  const [course, metadatacourse] = await sequelize.query(
   ...
   console.log(results)
  res.render('evaluationclasssearchresult',{ tag:"ngo" ,course:course, batchinfo:batchinfo,classlist:results,batchid:batchidn,dpt:dptn,programtag:"ngo",showtype:showtype})
});
...
const [courselist, metadata] = await sequelize.query(
    ...
    " and ngocourses.course_id='"+courseid+"'"
  );
  const [levelbased, metadatalevelbased] = await sequelize.query(
    " select * from ngobasedtrainees inner join studentmarklistlevelbaseds "+
    " on studentmarklistlevelbaseds.class_id = ngobasedtrainees.class_id where  "+
    " ngobasedtrainees.class_id = '"+ req.params.classname +"' and "+
    " ngobasedtrainees.trainee_id = studentmarklistlevelbaseds.student_id and "+
    " studentmarklistlevelbaseds.course_id = '"+courseid+"'"
  );   
  const [addinfo,addinfometa] = await sequelize.query(
    " select * from classindepts "+
...
router.post('/showclassevaluationindustry/(:classname)',ensureAuthenticated,async function(req,res){
  ...
  const [courselist, metadata] = await sequelize.query(
      ...
      " and industrycourses.course_id='"+courseid+"'"
    );
    const [addinfo,addinfometa] = await sequelize.query(
      " select * from classindepts "+
" inner join batches on batches.batch_id = classindepts.batch_id "+
...
" where classindepts.class_id='"+req.params.classname+"'"
    ...
    const [levelbased, metadatalevelbased] = await sequelize.query(
      "select * from industrybasedtrainees inner join "+
      " studentmarklistlevelbaseds on  "+
      " studentmarklistlevelbaseds.class_id = industrybasedtrainees.class_id " +
      " where industrybasedtrainees.class_id = '"+ req.params.classname +"' and "+
      " industrybasedtrainees.trainee_id = studentmarklistlevelbaseds.student_id and "+
      " studentmarklistlevelbaseds.course_id = '"+courseid+"'"
    );   
...
" levelbasedprogresses.class_id = levelbasedtrainees.class_id where "+
" levelbasedtrainees.class_id = '"+ req.params.classname +"' and "+
" levelbasedtrainees.trainee_id = levelbasedprogresses.student_id and "+
" levelbasedprogresses.course_id = '"+courseid+"'"
);     
...
const [levelbased, metadatalevelbased] = await sequelize.query(
  "select * from ngobasedtrainees inner join levelbasedprogresses on"+
  " levelbasedprogresses.class_id = ngobasedtrainees.class_id where"+
  " ngobasedtrainees.class_id = '"+ req.params.classname +"' and "+
  " ngobasedtrainees.trainee_id = levelbasedprogresses.student_id and "+
  " levelbasedprogresses.course_id = '"+courseid+"'"
);     
...
" where classindepts.class_id='"+req.params.classname+"'"
  ...
  const [levelbased, metadatalevelbased] = await sequelize.query(
    "select * from industrybasedtrainees inner join levelbasedprogresses "+
    " on levelbasedprogresses.class_id = industrybasedtrainees.class_id " +
    " where industrybasedtrainees.class_id = '"+ req.params.classname +"' "+
    " and industrybasedtrainees.trainee_id = levelbasedprogresses.student_id and "+
    " levelbasedprogresses.course_id = '"+courseid+"'"
  );     
